<?php 
    ...
    $sql_product = "SELECT product.prd_id, prd_name, prd_price, clr_name, siz_number, prd_image, prd_status, cat_name FROM product 
    INNER JOIN product_detail ON product.prd_id = product_detail.prd_id
    INNER JOIN category ON product.cat_id = category.cat_id INNER JOIN color ON product_detail.clr_id = color.clr_id
    INNER JOIN size ON product_detail.siz_id = size.siz_id ORDER BY prd_id DESC";
    ...
    $sql_pagination = "SELECT product.prd_id, prd_name, prd_price, clr_name, siz_number, prd_image, prd_status, cat_name, product_detail.clr_id, product_detail.siz_id FROM product 
     INNER JOIN product_detail ON product.prd_id = product_detail.prd_id
     INNER JOIN category ON product.cat_id = category.cat_id INNER JOIN color ON product_detail.clr_id = color.clr_id
     INNER JOIN size ON product_detail.siz_id = size.siz_id ORDER BY prd_id DESC LIMIT $start,$rowPerPage";
    ...
    <div class="row">
        <div class="col-lg-12">
            <div class="panel panel-default">
                ...
                <input type="text" id="myInput" onkeyup="myFunction()" placeholder="Tìm ID sản phẩm">
                    <table  data-toolbar="#toolbar" data-toggle="table" id="myTable">
                        <thead>
                            <tr>
                        ...
                        <tbody>
                            ...
                            ?>      
                                <tr>
                                    ...
                                    <td class="form-group">
                                        <a href="index.php?page=edit_product&prd_id=<?php echo $row['prd_id']; ?>&clr_id=<?php echo $row['clr_id']; ?>&siz_id=<?php echo $row['siz_id']; ?>" class="btn btn-primary"><i class="glyphicon glyphicon-pencil"></i></a>
